        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <Section id="concepts" icon="📐" title="Ключевые понятия">
            ...
            <Term term="Накопительный баланс (balance)">
              Сумма дельт всех предыдущих расчётов для данной позиции.
              Система извлекает её автоматически из истории:
              <span style={{ display: 'block', background: 'var(--bg)', borderRadius: 6, padding: '6px 10px', margin: '6px 0', fontFamily: 'monospace', color: 'var(--amber)' }}>
                balance = Σ delta (по всем прошлым расчётам этой позиции)
              </span>
              Именно баланс обеспечивает <em>точное</em> долгосрочное соблюдение долей:
            ...
            <Term term="Режим «Открыть последний мешок»">
              ...
              В этом режиме <code>factWeight = adjustedWeight</code>, а дельта становится нулём
              (баланс обнуляется). Удобно при финальной отгрузке партии.
            </Term>
          </Section>
          ...
          <Section id="formulas" icon="🔢" title="Формулы расчёта">
            ...
            />
            <Formula
              label="2. Баланс (накопленный из истории)"
              formula="balance = Σ delta_i  (по всем прошлым расчётам)"
              comment="Извлекается автоматически из таблицы CalculationHistory"
            />
            <Formula
            ...
            <div style={{ marginTop: 16 }}>
              <Tip>
                ...
                «излишек»). В режиме «открытый мешок» дельта равна нулю — баланс полностью обнуляется.
              </Tip>
            </div>
          ...
          <Section id="history" icon="📋" title="История и экспорт">
            ...
            <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: 16 }}>
              ...
              то есть его дельты перестают влиять на будущие расчёты.
            </p>
          ...
          <Section id="faq" icon="❓" title="Частые вопросы">
            {[
              ...
              {
                q: 'Для какой позиции рассчитывается баланс — одной или по компаниям?',
                a: 'Баланс глобальный — по всем расчётам для каждой позиции, независимо от компании. Это значит, что дельты разных клиентов суммируются. Если вам нужен раздельный учёт по клиентам — пока такой возможности нет.',
              },
              {
